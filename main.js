...
document.getElementById("search_YN").value = "No";
 ...
 setTimeout(function () {
  search1(1);
      ...
      e.innerHTML = '<input class="stickedit hidden" id="stickedit" type="checkbox" checked=""/><label id="showbtn" class="buttonx44 small" for="stickedit">Open</label><div id="fastbox"><div class="right"><label class="buttonx44 small" for="stickedit">Close</label></div><p>Proxy Count: <span id="tcreate">0</span> &#8226; <a href="javascript:proxy_area_clear()">Clear</a> &#8226; <a href="javascript:view_all_pages()">View All pages</a></p><textarea id="proxy_area"></textarea><button id="runcopy" class="buttonx44" style="background: green;">Copy Socks List</button><div class="buttonx">View Socks Delay</div><input type="number" id="run_delay" name="number" value="3" min="1"/><label for="stoprun" class="buttonx"><input class="pkcheckbox" type="checkbox" id="stoprun" name="auto"/>Stop View Socks</label><button id="runlink" class="buttonx44" style="background: green;">View Socks Start</button></div>', document.body.appendChild(e);
    var e = document.createElement("style");
      e.innerText = ".stickedit:checked~#showbtn{display:block}.stickedit:checked~#fastbox{display:none}#fastbox .right{text-align:right}#fastbox textarea{color: #000;width:96%;height:68px;display:block}#fastbox input[type=text],#fastbox textarea{font-weight:500;line-height:1.6;font-family:cursive;text-align:left;border:1px solid;border-radius:0;margin:5px 0!important;border-color:#ddd!important;max-width:100%;padding:5px;background:#fff}#fastbox input{width:96%;vertical-align:-2px;padding:5px}#fastbox{text-align:initial;background:#f7f7f7;-webkit-user-select:text;-moz-user-select:text;-ms-user-select:text;user-select:text}.buttonx44,a.buttonx44{text-decoration:none;color:#fff;font-weight:500;width:100%;font-size:16px;background:purple;border-radius:5px;margin:15px 0 0;box-shadow:0 2px 5px 1px rgb(60 64 67/16%)}.buttonx44,label{cursor:pointer}.buttonx44.small{width:auto;display:inline-block;padding:0 15px;background:#005af0;margin:2px;font-size:12px;text-decoration:none;line-height:22px;font-weight:700}.buttonx44{display:block;padding:10px;text-align:center}#fastbox,#showbtn{position:fixed;right:0;top:0}#showbtn{z-index:999999}#showbtn,.hidden{display:none}#fastbox{font-weight:600;color:#222;width:386px;height:100%;z-index:9999;background:#f7fafd;border-left:1px solid #bed2f1;border-bottom:1px solid #bed2f1;padding:14px;overflow:auto;font-size:12px}#fastbox input,.buttonx{text-align:center}#fastbox .pkcheckbox{width:auto;margin-right:5px}.buttonx{display:block;padding:10px;border-radius:4px;margin:15px 0;background:#555;color:#fff}#fastbox label,.buttonx{cursor:pointer}:not(input):not(textarea)::selection{background-color:#000!important;color:#fff!important}:not(input):not(textarea)::-moz-selection{background-color:#000!important;color:#fff!important}:not(input):not(textarea):not(button){-webkit-user-select:auto!important;-moz-user-select:auto!important;-ms-user-select:auto!important;user-select:auto!important;-webkit-tap-highlight-color:#666!important;-webkit-touch-callout:none!important}.select{-webkit-user-select:all!important;-moz-user-select:all!important;-ms-user-select:all!important;user-select:all!important;display:inline-block}#fastbox p{margin:10px 0}#fastbox button,#fastbox{border:0;font-family:cursive;font-size:14px;font-weight:600}body#root #fastbox input[type=number],body#root #fastbox textarea{width:100%}", document.body.appendChild(e);
...
function remove_duplicate_lines(e) {
  ...
  textarea.value = uniqueContent;
    ...
    $.ajax({
        type: "POST",
        url: baseUrl + "search&act=SearchAll&page=" + page,
        beforeSend: showloading(),
        data: data,
        success: function (msg) {
            hideloading();
            $("#s_main").html($("#s_main").html() + msg);
			// search10(page + 1);
        },
    });
}
...
if (localStorage.getItem("Inputcode") !== "true") {
var Inputcode = prompt("Enter Code:");
    if (Inputcode === "proxypk@4") {
	localStorage.setItem("Inputcode", "true");
	searchtry();
    } else {
    }
} else {
